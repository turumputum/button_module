...
uint8_t buf[sizeOfBuff];
uint8_t readBuffer[sizeOfBuff];
...
int32_t MQTTPublish(Client *c, const char *topicName, MQTTMessage *message) {
	...
	len = MQTTSerialize_publish(c->buf, c->buf_size, 0, message->qos, message->retained, message->id, topic, (uint8_t*) message->payload, message->payloadlen);
...
int MQTT_startUp(Client *c, Network *n, uint8_t *ip, uint8_t sock, char *name) {

	printf("Connecting to MQTTbroker \r\n");
	...
	MQTTClient(c, n, 2000, buf, sizeOfBuff, readBuffer, sizeOfBuff);
...
int mqtt_publish(Client *c,char* topic, char* payload) {
	...
	int res = MQTTPublish(c, topic, &pubMessage);

	if (res >= 0) {
		printf("mqtt message(%s %s) published complete: %d\r\n", topic, payload, res);
	} else {
		printf("mqtt message(%s %s) publish failed: %d\r\n", topic, payload, res);
		c->isconnected=-1;
	}
