	...
	)
{
  ...
  //nexttok = strtok_r((char*)buf," ", &reserve);
  nexttok = strtok_r((char*)buf," ", &reserve);
  if(!nexttok)
  {
  ...
  memset(request->BODY,0,MAX_URI_SIZE);
  strcpy((char *)request->BODY, reserve);
  memset(request->URI,0,MAX_URI_SIZE);
