...
void httpServer_run(uint8_t seqnum)
{
	...
	switch(getSn_SR(s))
	{
		...
		case SOCK_ESTABLISHED:
			...
			// HTTP Process states
			//uint8_t  tt = HTTPSock_Status[seqnum].sock_status;
...
static void http_process_handler(uint8_t s, st_http_request * p_http_request)
{
	...
	switch (p_http_request->METHOD)
	{
		...
		case METHOD_POST :
			...
			//mid(p_http_request->URI, "/", " HTTP", (char *)uri_buf);
			...
			{
				content_found = http_post_cgi_handler(uri_name, (struct _st_http_request *)p_http_request->BODY, http_response, &file_len);
...
void reg_httpServer_webContent(uint8_t * content_name, uint8_t * content)
{
	...
	web_content[total_content_cnt].content_name= content_name;
